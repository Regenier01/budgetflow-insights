...
const DEFAULT_INPUT_PATH = join('Orçado', 'Orça-Despesas Tributarias');
const FIXED_ATIVIDADE = 'DESP_ADM_TRIB';
const FIXED_DEPARTAMENTO = 'DESPESAS TRIBUTARIAS';
...
function mergeTributaryBatch(existingBatches, tributaryBatch) {
  const keyToReplace = `${FIXED_ATIVIDADE}::${FIXED_DEPARTAMENTO}`;

  const preserved = existingBatches.filter((batch) => {
    const key = `${normalizeText(batch.atividade)}::${normalizeText(batch.departamento)}`;
    return key !== keyToReplace;
  });

  return [...preserved, tributaryBatch];
}

function resolveExcelFileFromInput(inputPath) {
  const stats = statSync(inputPath);
  if (stats.isFile()) return inputPath;
  if (!stats.isDirectory()) {
    throw new Error(`Caminho invalido: "${inputPath}"`);
  }

  const files = readdirSync(inputPath).filter(
    (file) => /\.(xlsx|xls)$/i.test(file) && !file.startsWith('~$')
  );
  if (files.length === 0) {
    throw new Error(`Nenhum arquivo Excel encontrado em "${inputPath}".`);
  }

  const preferred = files.find((file) => normalizeText(file).includes('DESPESAS TRIBUTARIAS'));
  return join(inputPath, preferred || files[0]);
}

function run() {
  const targetInput = process.argv[2] || DEFAULT_INPUT_PATH;
  if (!existsSync(targetInput)) {
    throw new Error(`Caminho nao encontrado: "${targetInput}"`);
  }
  const targetFile = resolveExcelFileFromInput(targetInput);

  const rows = parseBudgetSheet(targetFile);
  const tributaryBatch = {
  ...
  const mergedBatches = mergeTributaryBatch(readExistingBatches(), tributaryBatch);
  writeFileSync(OUTPUT_FILE, buildOutputSource(mergedBatches));
  console.log(
    `Importacao de despesas tributarias concluida: 1 arquivo processado ("${targetFile}"). ` +
