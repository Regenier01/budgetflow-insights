...
const DEFAULT_ORCADO_DIR = join('Orçado', 'Orça-Agric');
...
function validateAgricolaFileNames(files) {
  ...
  throw new Error(
    `Arquivo(s) invalido(s) para agricola: ${invalidFiles.map((fileName) => `"${fileName}"`).join(', ')}. ` +
      `Use o padrao "NOME - CULTURA", exemplo: "LAVRINHA - SOJA.xlsx".`
  );
}
