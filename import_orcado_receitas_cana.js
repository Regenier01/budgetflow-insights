...
const OUTPUT_FILE = 'src/data/orcadoReceitaCanaImportData.ts';
const DEFAULT_ORCADO_DIR = join('Orçado_Receitas', 'Receita_Cana');
const FIXED_ATIVIDADE = 'CANA';
const FIXED_DEPARTAMENTO = 'TOTAL CANA';
const SYNTHETIC_FILE_NAME = 'RECEITA CANA (TOTAL)';
...
export const ORCADO_RECEITA_CANA_IMPORT_BATCHES: OrcadoImportBatch[] = ${JSON.stringify(batches, null, 2)};\n`;
}
...
function run() {
  ...
  files.forEach((fileName) => {
    ...
    totals.forEach((value, key) => {
      mergedTotals.set(key, (mergedTotals.get(key) || 0) + value);
    });
  });

  const rows = Array.from(mergedTotals.entries())
  ...
  writeFileSync(OUTPUT_FILE, buildOutputSource(receitaCanaBatches));
  console.log(
    `Importacao Receita Cana concluida: ${files.length} arquivo(s) processado(s) de "${targetDir}". ` +
    `Arquivo gerado: "${OUTPUT_FILE}" com 1 lote consolidado.`
  );
}
