...
const OUTPUT_FILE = 'src/data/orcadoReceitaAgricolaImportData.ts';
const DEFAULT_ORCADO_DIR = join('Orçado_Receitas', 'Receita_Agricola');
const FIXED_ATIVIDADE = 'AGRICOLA';
...
function parseCultureFromFilename(fileName) {
  return fileName
    .replace(/\.(xlsx|xls)$/i, '')
    .replace(/^RECEITA\s+/i, '')
...
export const ORCADO_RECEITA_AGRICOLA_IMPORT_BATCHES: OrcadoReceitaAgricolaImportBatch[] = ${JSON.stringify(batches, null, 2)};\n`;
}
...
function run() {
  ...
  const receitaAgricolaBatches = files.map((fileName) => {
    const totals = aggregateBudgetSheetByMonthAndGroup(join(targetDir, fileName));
    ...
    return {
      fileName,
      cultura: parseCultureFromFilename(fileName),
      atividade: FIXED_ATIVIDADE,
      rows,
    };
  });

  writeFileSync(OUTPUT_FILE, buildOutputSource(receitaAgricolaBatches));
  console.log(
    `Importacao Receita Agricola concluida: ${files.length} arquivo(s) processado(s) de "${targetDir}". ` +
    `Arquivo gerado: "${OUTPUT_FILE}" com ${receitaAgricolaBatches.length} lote(s).`
  );
}
