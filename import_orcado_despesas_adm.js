...
const DEFAULT_ORCADO_DIR = join('Orçado', 'Orça-Despesas Administrativas');
const COST_CENTER_MAPPING_FILE = 'src/data/costCenterMapping.ts';
...
function extractCostCenterFilters() {
  if (!existsSync(COST_CENTER_MAPPING_FILE)) return [];
  const source = readFileSync(COST_CENTER_MAPPING_FILE, 'utf-8');
  const filters = [];
  const entryPattern = /"([^"]+)"\s*:\s*\{[\s\S]*?"unidadeNegocio"\s*:\s*"([^"]+)"/g;

  let match;
  while ((match = entryPattern.exec(source)) !== null) {
    const centroCusto = match[1];
    const unidadeNegocio = match[2];
    if (!normalizeText(unidadeNegocio).includes('ADMINISTRACAO')) continue;
    filters.push(centroCusto);
  }

  return filters;
}

function validateAdministrativeFileNames(files) {
  const allowedCostCenters = extractCostCenterFilters();
  const allowedNormalized = new Set(allowedCostCenters.map((name) => normalizeLoose(name)));
  ...
  const sampleAllowed = allowedCostCenters.slice(0, 20).join(', ');
  throw new Error(
    `Arquivo(s) invalido(s) para despesas administrativas: ` +
      `${invalidFiles.map((item) => `"${item.fileName}"`).join(', ')}. ` +
      `O nome do arquivo (sem extensao) deve ser igual a um filtro de centro de custo do dashboard. ` +
      `Exemplos validos: ${sampleAllowed}${allowedCostCenters.length > 20 ? ', ...' : ''}`
  );
}
...
function run() {
  ...
  validateAdministrativeFileNames(files);

  const adminBatches = files.map((fileName) => ({
    fileName,
