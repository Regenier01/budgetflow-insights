...
import {
  tryParseSeringalOrcadoBudgetRow,
  getSeringalGrupoContabilN9DisplayLabel,
} from '@/data/seringalOrcadoGrupoContabilCodes';
import type { ExcelRow } from '@/types/budget';

describe('seringal orcado custos cascata (grupo → descrição, sem produto)', () => {
  beforeEach(() => {
  ...
  it('parses layout GRUPO_CONTABIL / CONTA / Descrição', () => {
    ...
    const parsed = tryParseSeringalOrcadoBudgetRow(row);
  ...
  it('rótulo N9 de exibição alinhado ao cubo', () => {
    expect(getSeringalGrupoContabilN9DisplayLabel('4.1.01.11', undefined)).toBe(
      '4.1.01.11-CUSTOS RURAIS'
    );
  });

  it('importa orçado seringal sem NOMEDEPTO (somente planilha)', () => {
    const rows: ExcelRow[] = [
      {
    ...
    const count = useBudgetStore
      .getState()
      .importOrcadoExcelRows(rows, '2026-04', 'COVOA - SERINGAL.xlsx');
    ...
    expect(leaf!.departamento).toBe('COVOA - SERINGAL');
