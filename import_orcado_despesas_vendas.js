...
const DEFAULT_INPUT_DIR = join('Orçado', 'Orça-DespVendas');
const DEPARTAMENTO_BY_ATIVIDADE = {
  PECUARIA: 'COMERCIALIZACAO DE TOUROS',
  AGRICOLA: 'COMERCIALIZACAO DE SEMENTES',
};
...
function parseAtividadeFromFilename(fileName) {
  const normalized = normalizeText(fileName);
  if (normalized.includes('PECUARIA')) return 'PECUARIA';
  if (normalized.includes('AGRICOLA')) return 'AGRICOLA';
  throw new Error(
    `Arquivo invalido para despesas com vendas: "${fileName}". ` +
      `Use nomes contendo "Pecuaria" ou "Agricola".`
  );
}

function parseDepartamentoFromAtividade(atividade) {
  return DEPARTAMENTO_BY_ATIVIDADE[atividade];
}
...
function mergeDespesasComVendasBatches(existingBatches, despesaBatches) {
  const incomingFileNames = new Set(despesaBatches.map((batch) => normalizeText(batch.fileName)));

  const preserved = existingBatches.filter((batch) => {
    return !incomingFileNames.has(normalizeText(batch.fileName));
  });

  return [...preserved, ...despesaBatches];
}
...
function run() {
  ...
  const despesaBatches = files.map((fileName) => {
    const atividade = parseAtividadeFromFilename(fileName);
    return {
      fileName,
      atividade,
      departamento: parseDepartamentoFromAtividade(atividade),
      rows: parseBudgetSheet(join(targetDir, fileName)),
    };
  });

  const mergedBatches = mergeDespesasComVendasBatches(readExistingBatches(), despesaBatches);
  writeFileSync(OUTPUT_FILE, buildOutputSource(mergedBatches));
  console.log(
    `Importacao de despesas com vendas concluida: ${despesaBatches.length} arquivo(s) processado(s) de "${targetDir}". ` +
